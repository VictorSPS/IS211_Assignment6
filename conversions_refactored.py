...
def convert(fromUnit, toUnit, value):
    """Converting given types and values to another type
    """
    fu = fromUnit[:3].lower()
    tu = toUnit[:3].lower()
    value = dec(value)
...
class ConverstionTests(unittest.TestCase):
    ...
    def testConversions(self):
        """Tests that our unit conversions work correctly"""
        for i in self.tvals:
            self.assertEqual(convert(i[0], i[1], i[2]), i[3])

    def testSameUnit(self):
        """Testinc conversion of units to the same units
        """
        for c in F1:
